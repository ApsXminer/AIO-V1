        ...
        Schema.findOne({ User: member.id }, async (err, data) => {
            if (data) {
                ...
                let embedLogs = new Discord.EmbedBuilder()
                    ...
                    .addFields(
                        { name: "<:CS_Profile:1371336333687259231>┆Banned By", value: `${interaction.user} (${interaction.user.tag})`, inline: true },
                    )
            ...
            if (data) {
                Schema.findOneAndDelete({ User: member.id }).then(() => {
                    ...
                    let embedLogs = new Discord.EmbedBuilder()
                        ...
                        .addFields(
                            { name: "<:CS_Profile:1371336333687259231>┆Unbanned By", value: `${interaction.user} (${interaction.user.tag})`, inline: true },
                        )
                        .setColor(client.config.colors.normal)
                        .setFooter({ text: client.config.discord.footer })
                        .setTimestamp();
