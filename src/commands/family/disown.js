    ...
    Schema.findOne({ Guild: interaction.guild.id, Parent: target.id }, async (err, data) => {
        if (data) {
            Schema.findOne({ Guild: interaction.guild.id, User: data.Parent }, async (err, data2) => {
                if (data2) {
                    client.embed({ title: `<:Blue_member:1371335093003747329> ・Disowned`, desc: `${author} has disowned <@!${data.Parent}>`, type: 'editreply' }, interaction);
            ...
            Schema.findOne({ Guild: interaction.guild.id, User: author.id }, async (err, data) => {
                if (data) {
                    if (data.Children.includes(target.username)) {
                        ...
                        client.embed({ title: `<:Blue_member:1371335093003747329> ・Disowned`, desc: `${author} has disowned <@!${target.id}>`, type: 'editreply' }, interaction);
                    }
                    else {
